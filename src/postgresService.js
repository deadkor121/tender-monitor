...
class PostgresService {
  constructor() {
    // Используем DATABASE_URL если доступен (Render), иначе локальные настройки
    const connectionString = process.env.DATABASE_URL;
    
    if (connectionString) {
      // Production: используем DATABASE_URL
      this.pool = new Pool({
        connectionString: connectionString,
        ssl: process.env.NODE_ENV === 'production' ? { rejectUnauthorized: false } : false
      });
      console.log('[PostgresService] Подключение через DATABASE_URL');
    } else {
      // Development: используем отдельные параметры
      this.pool = new Pool({
      ...
      console.log('[PostgresService] Подключение через локальные параметры');
    }
